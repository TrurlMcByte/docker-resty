...
function _M.get_connection(self)
  ...
  if self.options["socket"] then
    ...
    ok, err = connection:connect(self.options["host"], self.options["port"], self.options["options_table"] )
  end
  if not ok then
    err = "redis: connect error: " .. err
    return false, err
  end
